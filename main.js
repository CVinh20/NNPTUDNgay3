...
function convertDataToHTML(product) {
    ...
    // Display all 3 images in carousel format
    const imagesCarousel = product.images && product.images.length > 0
        ? `<div class="product-carousel">
                <div id="carousel-${product.id}" class="carousel slide carousel-sm">
                    <div class="carousel-inner">
                        ${product.images.slice(0, 3).map((img, idx) => `
                            <div class="carousel-item ${idx === 0 ? 'active' : ''}">
                                <img referrerpolicy="no-referrer" src="${img}" class="d-block w-100" alt="Image ${idx + 1}" style="height: 180px; object-fit: cover;">
                            </div>
                        `).join('')}
                    </div>
